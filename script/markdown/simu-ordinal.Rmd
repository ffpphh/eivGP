...
params:
  ...
  save.filename: 'output/MCMC/EIV-ordinal-cos-10.rds'
---
...
source(paste0(params$dir.code, 'basics-ordinal.R'))
...
```{r fig.cap = 'True response', echo=FALSE}
...
ggplot(u_df, aes(x = u, color = class, shape = observed)) + 
  ...
  my_theme
ggsave(paste0(params$fig.code, 'Scatter-y-vs-u.pdf'), plot = ggfig, width = 8, height = 6)
...
ggplot(u_df[u_df$observed=="TRUE",], aes(x = u, y = y, color = class, shape = observed)) +
  ...
  my_theme
ggsave(paste0(params$fig.code, 'Scatter-y-vs-u-obs.pdf'), plot = ggfig, width = 8, height = 6)
...
ggplot(u_df, aes(x = c, y = y, color = class)) +
  ...
  my_theme
ggsave(paste0(params$fig.code, 'Scatter-y-vs-c.pdf'), plot = ggfig, width = 8, height = 6)
```
...
#ggfig <- 
  ...
  labs(color = "Class", shape = 'Observation') + 
  my_theme
#ggsave(paste0(params$fig.code, 'Scatter-u-vs-x.pdf'), plot = ggfig, width = 8, height = 6)
```
...
ggsave(paste0(params$fig.code, 'Scatter-y-x-c.pdf'), plot = ggfig[[1]], width = 8, height = 6)
ggsave(paste0(params$fig.code, 'Scatter-y-x-u-obs.pdf'), plot = ggfig[[2]], width = 8, height = 6)
```
...
The selected lengthscale for $\mathbf{x}$ is `r eb.hyper.opm[1:nc]`,  the lengthscale for $\mathbf{U}$ is `r eb.hyper.opm[(nc+1):(nc+1)]`, and the GP signal-to-noise ratio is `r eb.hyper.opm[nc+1+1]`.
