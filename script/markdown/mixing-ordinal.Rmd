...
params:
  ...
  results.file: 'output/MCMC/EIV-ordinal-cos-10.rds'
---
...
ggplot(u_df, aes(x = U.true, y = U.pos.mean, color = class)) +
  ...
  my_theme
ggsave(paste0(params$fig.code, 'Imputed-u-vs-true-u.pdf'), plot = ggfig, width = 8, height = 6)
...
ggplot(u_df, aes(y = f.true, col = Coverage)) +
  ...
  my_theme
ggsave(paste0(params$fig.code, 'Imputed-u-quantile.pdf'), plot = ggfig, width = 8, height = 6)
```
...
ggplot(f_df, aes(x = u)) + 
  ...
  my_theme
ggsave(paste0(params$fig.code, 'prediction-f-GP.pdf'), plot = ggfig, width = 8, height = 6)
...
ggplot(f_eiv_df, aes(x = u)) + 
  ...
  lims(y = c(min(f_df$f.lq, f_eiv_df$f.lq, f_df$f.true), max(f_df$f.uq, f_eiv_df$f.uq, f_df$f.true))) + 
  my_theme
ggsave(paste0(params$fig.code, 'prediction-f-EIV.pdf'), plot = ggfig, width = 8, height = 6)
...
Oracle prediction (assuming the DGP is true and using true parameters).
...
for (idx in 1:length(c.star)) {
  ...
  #ggsave(paste0(params$fig.code, 'prediction-', idx, '.pdf'), plot = grid.arrange(grid.arrange(grobs = plot_list, nrow = 1)), width = 8*3, height = 6)
}
```
