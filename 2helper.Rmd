...
#' @param Gibbs.other List containing other configurations (e.g., seed).
#' 
#' @return A list of initial values (e.g., U0, Tau0).
#' @export
get.initial <- function(input.type, Gibbs.data, Gibbs.other = list()) {
  ...
  if (input.type == "ordinal") {
    return(get.initial.ordinal(Gibbs.data, Gibbs.other))
  } else {
    return(get.initial.nominal(Gibbs.data, Gibbs.other))
  }
}


#' Generate initial values for Ordinal Models
get.initial.ordinal <- function(Gibbs.data, Gibbs.other) {
...
#' Generate initial values for Nominal Models
get.initial.nominal <- function(Gibbs.data, Gibbs.other) {
...
###Predict Oracle
```{r}
#' Unified Oracle Predictor Function
#' 
#' @param input.type String. "ordinal" or "nominal".
...
predict.oracle.nominal <- function(Gibbs.data, Gibbs.param, Gibbs.other) {
  ...
  uniform <- if(!is.null(Gibbs.other$uniform)) Gibbs.other$uniform else FALSE
  n.mc <- if(!is.null(Gibbs.other$n.mc)) Gibbs.other$n.mc else 1000
  ...
  for (idx in 1:n.mc) {
    ...
    for (i in 1:n) {
      ...
      while (TRUE) {
        ...
        # Determine category based on true classifier f
